...
public class Creatures_Character_Controller : MonoBehaviour {
	...
	*/
		...
		*/
		...
		if(anim.GetBool("dead")==false){
			...
			if(shield_mode==false){
				...
				PlayMove (Input.GetAxis ("Horizontal"));
			}
			*/
	...
	public void PlayAttack(bool mode){anim.SetBool ("attack", mode);}
	...
	void PlayJump(){
		if(anim.GetBool("jump")==false){//only once time each jump
			GetComponent<Rigidbody2D>().AddForce(new Vector2(0f,140));
			anim.SetBool("jump",true);
		}
	}

	public void PlayMove(float move){
